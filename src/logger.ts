...
export interface IObservableLogger {
	handleObservableChanged(observable: ObservableValue<any>, info: IChangeInformation): void;
...
export class ConsoleObservableLogger implements IObservableLogger {
	...
	private readonly changedObservablesSets = new WeakMap<object, Set<IObservable<unknown>>>();

	formatChanges(changes: Set<IObservable<unknown>>): ConsoleText | undefined {
		if (changes.size === 0) {
			return undefined;
		}
		return styled(
	...
	handleDerivedCreated(derived: Derived<unknown>): void {
		...
		derived.handleChange = (observable) => {
			this.changedObservablesSets.get(derived)!.add(observable);
			return existingHandleChange.apply(derived, [observable]);
		};
	}
	...
	handleAutorunCreated(autorun: AutorunObserver): void {
		...
		autorun.handleChange = (observable) => {
			this.changedObservablesSets.get(autorun)!.add(observable);
			return existingHandleChange.apply(autorun, [observable]);
		};
	}
